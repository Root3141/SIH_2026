...
Complete documentation of the SkyGuard anomaly detection system,
covering statistical, spatial, anomaly injection, and evaluation
modules.
...
This file implements statistical methods to detect anomalies in weather
station observations (temperature, humidity, pressure). Here's a
detailed breakdown:
...
-   **`StatisticalConfig`**: Stores detection parameters like z-score
    thresholds, time windows (24h, 168h), and persistence tolerances
-   **`StatisticalCalibration`**: Holds pre-computed baselines and
    thresholds from training data

### **Detection Methods**

1.  **Range Anomalies** (`detect_range_anomalies`)
    -   Checks if values fall outside normal bounds (e.g., temperature:
        -10°C to 55°C)
...
2.  **Rate of Change (ROC)** (`detect_roc_anomalies`)
    ...
    -   Compares absolute change to hourly percentile thresholds from
        calibration
3.  **Z-Score Level Detection** (`calculate_rolling_zscore`)
    -   Calculates how many standard deviations a value deviates from
        rolling mean
    -   Uses grouped rolling windows per station to detect sustained
        deviations
...
`_run_self_test()` generates synthetic weather data and injects
anomalies to verify detection works correctly.
...
This file detects anomalies by comparing each weather station to nearby
stations at the same timestamp. It identifies readings that deviate
significantly from spatially interpolated expected values.
...
-   **`SpatialConfig`**: Controls detection behavior
    ...
    -   Separate percentile thresholds for humidity (more variable)
        vs. temperature/pressure
    -   `min_neighbors_required`: Minimum valid neighbors to make a
...
-   **`SpatialCalibration`**: Pre-computed station metadata
    ...
    -   `residual_thresholds`: P95 and P99 percentile thresholds per
        station/variable

### **Distance Calculation**

**`haversine_km`**: Computes great-circle distance between two lat/lon
coordinates using the Haversine formula. Returns distance in kilometers.
...
1.  Creates pivot table: rows=timestamps, columns=station_ids,
    values=variable
2.  For each station, computes Inverse Distance Weighting (IDW)
    interpolation:
    -   Multiplies neighbor values by their normalized weights
    -   Handles missing data by tracking valid neighbors per timestamp
    -   Returns expected values using weighted average of available
        neighbors
...
2.  For each variable and station:
    ...
    -   Humidity uses stricter thresholds (P99.0 vs P99.5 for other
        variables)
...
2.  **Severity Assignment**:
    ...
    -   Sets severity: `"anomaly"` (1+ P99) → `"suspicious"` (1+ P95) →
        `"normal"`
    -   Humidity anomalies don't trigger alerts independently (they
        inform but don't drive severity)

### **Key Design Decisions**

-   **Humidity special handling**: Higher percentile thresholds (99% vs
    95%) because humidity is naturally more variable
-   **Non-humidity focus**: Only non-humidity P95/P99 counts drive
    severity (humidity is informative but not decisive)
-   **Minimum neighbors requirement**: Ensures predictions are reliable
    before flagging
-   **Regional vs. local anomalies**: IDW catches isolated spikes
    (single station deviates) but not coordinated regional events (all
    stations move together)

### **Test Cases**

`_run_self_test()` validates two scenarios:

-   **Case A (Isolated spike)**: Station AWS_A +15°C alone → Flagged as
    anomaly ✓
-   **Case B (Regional event)**: All 4 stations +6°C together → NOT
    flagged (normal regional variation) ✓

This demonstrates the detector correctly distinguishes sensor
malfunctions from legitimate weather patterns.
...
This file synthetically injects realistic anomalies into clean weather
station data for testing detector performance. It creates ground-truth
labeled datasets where you know exactly what anomalies were introduced.
...
-   `anomaly_rate`: Target percentage of observations to contaminate
    (default 1%)
-   `min_duration` / `max_duration`: Event length range (1-24 hours,
    except spikes always = 1)
-   `anomaly_type_weights`: Distribution of anomaly types (e.g., spike
    20%, offset 20%)
-   `severity_weights`: Distribution across low/medium/high
    (25%/50%/25%)
-   `prevent_overlap`: Prevent multiple anomalies on same (station,
    variable) pair
-   `station_local_only`: Constrain anomalies to individual stations

**`AnomalyEvent`**: Records metadata for each injected anomaly: type,
severity, station, variable, time window, magnitude, affected row count.

### **Variable Parameters**

**`VARIABLE_PARAMETERS`** dictionary defines severity-specific
magnitudes for each variable:
...
Each variable also has physical bounds (temperature: -10°C to 55°C,
humidity: 0-100%, pressure: 950-1060 hPa).
...
2.  **Initialization**:
    ...
    -   Creates tracking columns for original values and anomaly
        metadata
...
4.  **Overlap Prevention**:
    ...
    -   `_find_available_start`: Searches for duration-length window
        without conflicts
    -   `_mark_occupied`: Tracks which indices are used
    -   Prevents same (station, variable) pair from having simultaneous
        anomalies
...
5.  **Output Columns Added**:
    ...
    -   `synthetic_anomaly_type`: Type string (handles multiple overlaps
        with `|` separator)
...
-   **`_sample_weighted_choice`**: Probabilistic selection from weighted
    dict
-   **`_sample_magnitude`**: Draws magnitude from range for (variable,
    type, severity)
-   **`_random_sign`**: Returns ±1 randomly
-   **`_get_station_indices`**: Builds dict of row indices per station
    for efficient sampling
-   **`_clip_to_physical_range`**: Prevents unrealistic values (e.g.,
    -50°C temperature)
...
1.  [x] Actual rate matches target (within 1%)
2.  [x] STUCK events have zero variance
3.  [x] DROPOUT events are all NaN
4.  [x] SPIKE events have duration=1
5.  [x] No overlapping events per (station, variable) pair
6.  [x] All values respect physical range bounds

------------------------------------------------------------------------

## **Key Design Decisions**

-   **Severity-based magnitudes**: High severity spikes are more extreme
    than low severity
-   **Variable-specific parameters**: Humidity changes are larger
    (0-100% scale) than pressure (950-1060 range)
-   **Prevent overlap option**: Ensures controlled contamination for
    evaluation
-   **Event tracking**: UUID + metadata enables precise ground truth for
    testing detectors
-   **Original value backup**: Allows contamination rollback if needed
-   **Multiple anomalies per observation**: Handles edge cases by
    pipe-separating values
...
This file comprehensively evaluates the statistical anomaly detector's
performance using synthetic ground-truth anomalies. It follows a
complete pipeline: calibrate → inject anomalies → detect → analyze →
report.

## **High-Level Workflow**

1.  **Load datasets**: Historical (calibration) + Present (evaluation)
2.  **Calibrate detector**: Learn baselines/thresholds from clean
    historical data
3.  **Inject anomalies**: Add synthetic ground-truth anomalies to
    evaluation data
...
-   Prints: stations calibrated, z-score windows, percentiles,
    thresholds
...
-   Returns results with columns: `statistical_alert`,
    `statistical_severity_label`, etc.
...
-   Compares ground truth (`is_anomaly`) vs. predictions
    (`statistical_alert`)
-   Calculates confusion matrix: TP, FP, FN, TN
-   Returns:
    -   **Precision**: TP / (TP + FP) --- of flagged observations, how
        many are truly anomalous?
    -   **Recall**: TP / (TP + FN) --- of actual anomalies, how many did
        we detect?
    -   **F1 Score**: Harmonic mean of precision/recall
    -   **False Positive Rate**: FP / (FP + TN) --- of clean
        observations, what % did we incorrectly flag?
...
3.  For each event, calculates:
    -   Anomaly type, variable, severity, duration (number of
        observations)
    -   Was event detected? (any row in event has
        `statistical_alert=True`)
    -   How many observations in event were detected?
4.  Returns: total events, detected events, missed events, event-level
    recall
5.  **Event-level recall**: What fraction of anomaly *events* had ≥1
    detection?
    -   Different from observation-level recall (% of observations
        detected)
...
-   Returns sorted DataFrame showing which anomaly types are
...
-   Groups injected anomalies by variable (temperature, humidity,
    pressure)
-   Calculates recall per variable
-   Shows which variables are detected better
-   Example: Temperature anomalies might have higher recall than
    humidity
...
-   Lists individual detector columns: `temperature_range_anomaly`,
    `humidity_zscore_6_anomaly`, etc.
...
-   Groups detector outputs into four families: range, ROC, level
    (z-score), persistence
-   Shows how many observations triggered each family
-   Identifies which evidence type is most common
-   Example: "range" family might fire on 8.2% of observations, "level"
    on 2.1%
...
-   Shows which stations have highest alert rates (might indicate sensor
    issues or installation location)

### **Severity Distribution**

**`analyze_severity_distribution()`**:

-   Groups by `statistical_severity_label` (normal, suspicious, anomaly,
    critical)
-   Counts observations and injected anomalies per severity level
-   Shows: how many anomalies fell into each bucket?
-   Example: 45 critical (all injected), 89 anomaly (78 injected), 1230
    suspicious (2 injected)
...
**`generate_plots()`**: Generates 4-panel figure saved to
`statistical_detector_evaluation.png`:
...
**`save_results()`** saves 10 output files to `RESULTS_STATISTICAL_DIR`:
  ...
  `event_metrics.csv`                     Per-event detection: type, variable, severity,
                                          duration, detected?

  `performance_by_anomaly_type.csv`       Recall for
                                          spike/offset/drift/stuck/noise/dropout/rate_change
  ...
  `severity_distribution.csv`             Distribution across
                                          normal/suspicious/anomaly/critical

  `injection_log.csv`                     Details of all injected anomaly events

  `anomaly_predictions.csv`               Rows with `is_anomaly=True` OR
                                          `statistical_alert=True`
...
-   **High Precision, Low Recall**: Detector is conservative, rarely
    flags things. When it does, it's usually right. But misses many real
    anomalies.
-   **Low Precision, High Recall**: Detector is aggressive, catches most
    real anomalies but produces many false alarms.
-   **Goal**: Balance both (high F1 score)

**Event-Level vs. Observation-Level**:

-   **Event-level recall**: Did we detect the anomaly *somewhere* in its
    duration?
-   **Observation-level recall**: What percentage of individual
    anomalous readings did we catch?
...
This modular evaluation framework enables comprehensive assessment of
...
This file comprehensively evaluates the spatial anomaly detector's
performance using the same methodology as the statistical evaluator, but
tailored for spatial consistency checks (comparing stations to
neighbors).
...
1.  **Load datasets**: Historical (calibration) + Present (evaluation)
2.  **Calibrate detector**: Build neighbor network and residual
    thresholds from clean data
3.  **Inject anomalies**: Add synthetic ground-truth anomalies to
    evaluation data
4.  **Run detector**: Apply spatial detector to contaminated data
5.  **Evaluate**: Calculate metrics at observation/event/anomaly-type
    levels
6.  **Test regional events**: Sanity check that shared events aren't
    misclassified
7.  **Report**: Save CSVs + generate plots
...
-   Loads `HISTORICAL_PARQUET` and `PRESENT_PARQUET` (same as
...
Key difference from statistical: **requires geographic coordinates** to
...
-   Compares ground truth (`is_anomaly`) vs. predictions
    (`spatial_alert`)
-   Calculates: TP, FP, FN, TN, precision, recall, F1, false positive
    rate
...
4.  Calculates **event-level recall**: (detected events) / (total
    events)
...
2.  Groups by `anomaly_type` (spike, offset, drift, stuck, noise,
    dropout, rate_change)
...
Shows which variables (temp/humidity/pressure) spatial detector handles
best.
...
2.  Counts how many were incorrectly flagged as alerts
    (`spatial_alert=True`)
3.  Calculates false positive rate: FP / (FP + TN)
4.  Breaks down FP counts by station
5.  Prints: total clean, FP count, FP rate, FP by station

Identifies stations with high false positive rates (over-sensitive or
noisy).
...
1.  Groups by `spatial_severity_level` (normal, suspicious, anomaly,
    critical)
...
This is a critical **sanity check** that spatial detector correctly
distinguishes:

-   **Isolated anomalies** (one station deviates) → Should be flagged ✓
-   **Regional events** (all stations move together) → Should NOT be
    flagged ✓

Process:

1.  Randomly selects a timestamp in middle of dataset
2.  Adds shared +6°C to all stations at that timestamp (simulating
    regional weather event)
3.  Runs spatial detector on modified data
4.  Counts alerts during regional event
5.  **Expected**: 0 alerts (all stations agree, so IDW interpolation
    matches)
6.  **Result**: Prints PASS/WARNING

This validates that detector catches sensor malfunctions but ignores
...
**`generate_plots()`** creates 4-panel figure
(`spatial_detector_evaluation.png`):
...
4.  **Severity Distribution** (bottom-right):
    -   Bar chart: severity_level (normal/suspicious/anomaly/critical)
        vs. observation count
...
**`save_results()`** saves 8 output files to `OUTPUT_DIR`
(results/spatial/):
  ...
  `event_metrics.csv`                 Per-event detection: type, variable, duration,
                                      detected?
  ...
  `regional_event_test.csv`           Results from sanity test (stations during regional
                                      event)
...
## **Key Differences from Statistical Evaluator**
  ...
  --------------- --------------------------- ---------------------------
  **Calibration   Time series patterns        Geographic coordinates
  data**                                      

  **Detection     Temporal baselines (hourly) Spatial interpolation (IDW)
  logic**                                     

  **Anomaly       Per-variable deviation from Per-station deviation from
  detection**     baseline                    neighbors

  **Regional      May flag all stations       Should NOT flag (all
  events**                                    neighbors agree)

  **Sanity test** Isolated spike vs. noise    Regional event vs. local
                                              spike

  **Strength**    Detects temporal patterns   Detects spatial
                                              inconsistencies

  **Weakness**    Misses coordinated regional Misses coordinated regional
                  events                      events
  -----------------------------------------------------------------------
...
-   **High Precision, Low Recall**: Detector is conservative; few false
    alarms but misses anomalies
-   **Low Precision, High Recall**: Detector is aggressive; catches most
    anomalies but many false alarms
-   **F1 Score**: Balanced metric between precision and recall (target:
    \>0.8)
-   **Event-level recall \> Observation-level recall**: Normal (catching
    ≥1 observation per event is easier than catching all)
-   **Spike/Stuck detection \>95%**: Spatial detector excellent for
    obvious local anomalies
-   **Drift detection \<70%**: Harder to detect gradual changes (may
    blend with neighbors)
-   **Regional event test PASS**: Detector correctly ignores shared
    weather patterns
...
This comprehensive guide covers all major components of the SkyGuard
anomaly detection system for weather station observations. Each module
is designed to work together: statistical detection identifies temporal
anomalies, spatial detection identifies geographic inconsistencies,
anomaly injection creates ground-truth datasets, and evaluation
frameworks assess detector performance comprehensively.

------------------------------------------------------------------------

# Prototype Integration Reference

The final SkyGuard prototype adds an offline-preparation/runtime
boundary around the three existing detectors. The detector
implementations remain independent; the integration files prepare a
reproducible 2026 benchmark, freeze reusable detector state, combine
detector outputs, and provide a stable dataset for the Streamlit
frontend.

## Prototype File and Artifact Map
...
The final preparation produces:
...
The generated files are distinct from the experimental evaluation
outputs under `results/statistical/`, `results/spatial/`,
`results/lstm_autoencoder/`, and `results/statistical_spatial_lstm/`.

## `src/skyguard/simulation/create_final_dataset.py`

This module creates the fixed, reproducible 2026 benchmark used by the
prototype.

### Responsibilities

-   Load the clean 2026 present dataset.
-   Prefer `data/raw/ncr_weather_2026_present.parquet`; fall back to the
    CSV source when the Parquet companion is unavailable.
-   Apply the existing `inject_anomalies()` implementation.
-   Use the final prototype configuration: anomaly rate **2%**, random
    seed **42**, and temperature/humidity/pressure.
-   Preserve the injector's `synthetic_*` ground-truth fields.
-   Add `is_anomaly`, `anomaly_id`, `anomaly_type`, `anomaly_variable`,
    and `anomaly_severity`.
-   Replace injector UUIDs with stable sequential event IDs so the
    benchmark and event log are reproducible across runs.
-   Save the benchmark data and injection log.
...
The injected dataset is the prototype's data input. The event log is
retained for evaluation and explanation and is not detector input.
Ground-truth columns must never be used to make runtime detector
decisions.

## `src/skyguard/fusion/calibrate.py`

This module creates the reusable detector state required by the
prototype. It is a packaging/calibration boundary, not a new
anomaly-detection algorithm.

### Calibration source

Only clean historical data is used:

``` text
2023-01-01 through 2025-12-31
```

The 2026 benchmark and its synthetic labels are excluded from
calibration.

### Responsibilities

-   Calibrate the statistical detector using
    `calibrate_statistical_detector(...)`.
-   Calibrate the spatial detector using
    `calibrate_spatial_detector(...)`.
-   Reuse the already-frozen LSTM calibration state rather than
    retraining it.
-   Package detector state into stable runtime artifacts.
-   Record the calibration period and prototype fusion configuration.
...
`statistical.pkl` and `spatial.pkl` contain the detector configuration
and calibration objects required for inference. Statistical calibration
includes values such as `roc_thresholds` and `hourly_baselines`; spatial
calibration includes `neighbors` and `residual_thresholds`.

`lstm.pkl` is a copy of the frozen LSTM calibration cache. The frozen
state contains the trained model, normalization state, configuration,
calibration thresholds, and relevant calibration metadata.

`fusion_config.json` records the historical calibration period, detector
artifact names, and the prototype 2-of-3 fusion policy.

## `src/skyguard/fusion/fusion.py`

This is the runtime boundary between the detector layer and the
frontend.
...
The function loads saved detector artifacts, restricts detector input to
weather observations, runs the frozen Statistical, Spatial, and LSTM
Autoencoder detectors, converts their alerts into a vote count, applies
the deterministic 2-of-3 policy, and returns the input plus
detector/fusion fields.
...
The result retains the input fields and detector-specific outputs and
adds:
...
Detector-specific names remain prefixed, including:
...
This is a deterministic prototype presentation policy, not a newly
calibrated ML claim.

### Final confidence

`final_confidence` is `detector_votes / 3`. It represents detector
agreement, not a calibrated probability. The UI should preferably
display `Detector agreement: N / 3`.

Synthetic ground-truth fields may remain in evaluation results, but they
are excluded from detector input and runtime decisions.

## `data/synthetic/skyguard_demo_2026.parquet`

The stable, reproducible injected 2026 benchmark used as the input to
final prototype inference. It retains ground-truth metadata for offline
evaluation and explicitly labeled demo annotation.

## `data/synthetic/skyguard_demo_2026_log.parquet`

The event-level log produced alongside the benchmark. It records
injected anomaly events and metadata for evaluation and explanation and
is not part of the detector decision path.

## `artifacts/skyguard_v1/`

The frozen runtime state for the prototype:
...
These artifacts allow inference to reuse calibration/training state
rather than learn from the 2026 benchmark.

## `results/prototype/skyguard_demo_2026_results.parquet`

The stable presentation dataset consumed by Streamlit. It contains the
precomputed weather/evaluation fields, detector outputs, detector vote
count, final alert, final severity, and agreement-based confidence.

The dashboard reads this artifact for replay, station status, detector
agreement, severity, and alert history. It must not retrain,
recalibrate, inject anomalies, run detector inference, or modify the
artifact at startup.

## Frontend Integration Boundary

The final frontend reads the precomputed result artifact rather than
maintaining a second anomaly-detection implementation. The Streamlit
layer is responsible for presentation such as station status, maps,
current readings, alert details, detector agreement, anomaly timelines,
and recent sensor-health summaries.

For the demo, timestamps may be replayed sequentially to create a
live-stream effect, but the displayed anomaly decisions come from the
precomputed SkyGuard results.

Ground-truth annotations may be shown only in an explicitly labeled
evaluation/demo view and must not be presented as detector predictions.

## Prototype Data/Model Boundary
...
Clean historical 2023–2025
  ...
  Frozen detector artifacts
          │
          │
Clean 2026 ──> fixed anomaly injection
          │
          ▼
  Injected 2026 benchmark
          │
          ▼
...
Historical data is used for calibration/training, the 2026 benchmark is
used for evaluation/demo inference, and synthetic ground-truth labels
are never used to produce runtime decisions.
